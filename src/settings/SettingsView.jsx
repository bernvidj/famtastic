...
import { Save, UserPlus, LogOut, Shield, Coins, Users, Home } from 'lucide-react';
...
export function SettingsView({ familyId, member, members, onUpdate, onLogout }) {
  ...
  if (loading) return <div style={styles.page}><p style={styles.loadingText}>Laddar...</p></div>;

  return (
    <div style={styles.page}>
      {toast && <div style={styles.toast}>{toast}</div>}

      <div style={styles.header}><h1 style={styles.pageTitle}>Inställningar</h1></div>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.card}>
          <div style={styles.cardHeaderRow}>
            <h2 style={styles.cardTitle}><Users size={18} /> Medlemmar</h2>
            {isParent && (
...
const styles = {
  ...
  addForm: { padding: 14, background: C.bg, borderRadius: 12, marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8 },
  colorRow: { display: 'flex', gap: 6 },
  colorDot: { width: 28, height: 28, borderRadius: 14, cursor: 'pointer', padding: 0 },
  sectionLabel: { fontFamily: F.heading, fontSize: F.sizes.xs, fontWeight: F.weights.bold, color: C.textMuted, margin: '8px 0 6px', textTransform: 'uppercase', letterSpacing: 0.5 },
