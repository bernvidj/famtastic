...
export function ChildMoneyView({ familyId, memberId, transactions, goals, familyGoals, members, onReload }) {
  ...
  async function handleConfirm() {
    ...
    const { data, error } = await supabase.rpc('child_move_money', {
      p_family_id: familyId, p_member_id: memberId,
      p_amount_ore: amountOre, p_type: moveTo,
    ...
    <div style={styles.page}>
      ...
      <div style={styles.balanceCard}>
        <div style={styles.balanceLeft}>
          <div style={styles.balanceIconWrap}><Wallet size={24} color="#fff" /></div>
          <div>
      ...
      <div style={styles.jarsGrid}>
        {walletBalance > 0 && (
          <button onClick={() => startMove('withdrawal')} style={{ ...styles.jar, borderColor: C.error, cursor: 'pointer' }}>
            <div style={{ ...styles.jarIcon, background: C.errorLight }}><ArrowUpCircle size={28} color={C.error} /></div>
            <span style={styles.jarLabel}>Ta ut</span>
            <span style={{ ...styles.jarAmount, color: C.error }}>Få pengar</span>
        ...
        <button onClick={() => startMove('saving')} style={{ ...styles.jar, borderColor: C.secondary, cursor: 'pointer' }}>
          <div style={{ ...styles.jarIcon, background: C.secondaryLight }}><PiggyBank size={28} color={C.secondary} /></div>
          <span style={styles.jarLabel}>Spargris</span>
          <span style={{ ...styles.jarAmount, color: C.secondaryDark }}>{goals.length > 0 ? `${goals.length} mål` : 'Skapa mål'}</span>
        ...
        {familyGoals.length > 0 && (
          <button onClick={() => startMove('family_goal')} style={{ ...styles.jar, borderColor: C.accent, cursor: 'pointer' }}>
            <div style={{ ...styles.jarIcon, background: C.accentLight }}><Target size={28} color={C.accent} /></div>
            <span style={styles.jarLabel}>Familjemål</span>
            <span style={{ ...styles.jarAmount, color: '#92400E' }}>{familyGoals[0]?.icon} {familyGoals[0]?.title}</span>
              ...
              <div key={g.id} style={styles.goalCard}>
                <div style={styles.goalTop}>
                  ...
                  <div style={styles.goalInfo}>
                    ...
                    <span style={styles.goalAmountText}>{formatKr(saved)} av {formatKr(g.target_amount)}</span>
                  </div>
                  <span style={styles.goalPct}>{Math.round(pct)}%</span>
                </div>
                <div style={styles.goalBarBg}>
                  <div style={{ ...styles.goalBarFill, width: `${pct}%`, background: pct >= 100 ? C.success : `linear-gradient(90deg, ${C.secondary}, ${C.accent})` }} />
                </div>
              </div>
            );
          })}
...
const styles = {
  ...
  balanceCard: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px', borderRadius: 20, background: `linear-gradient(135deg, ${C.primary}, #E85D04)`, marginBottom: 16, boxShadow: '0 4px 16px rgba(249,115,22,0.3)', boxSizing: 'border-box', width: '100%' },
  balanceLeft: { display: 'flex', alignItems: 'center', gap: 12 },
  balanceIconWrap: { width: 48, height: 48, borderRadius: 14, background: 'rgba(255,255,255,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  ...
  jar: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '20px 16px', borderRadius: 20, border: '2.5px solid', background: C.bgCard, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', width: '100%', textAlign: 'center', boxSizing: 'border-box' },
  ...
  moveCard: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, padding: 16, background: C.bgCard, borderRadius: 16, border: `1.5px solid ${C.borderLight}`, marginBottom: 20 },
  ...
  confirmBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: C.primary, color: '#fff', fontSize: F.sizes.md, fontWeight: F.weights.bold, fontFamily: F.heading, cursor: 'pointer', minHeight: 52 },
