...
export function ChoresView({ familyId, member, members }) {
  ...
  function isScheduledForToday(c) {
    ...
    // Non-recurring without scheduled_date = always show (legacy/permanent chore)
    return true;
  }

  function weekTotal(chore) {
    if (chore.scheduled_date) return 1; // one-off = 1
  ...
  function getTodayChores() {
    return chores.filter(c => {
      ...
      // Check scheduling (date, day-of-week, until)
  ...
  // Pool chores: only unclaimed ones (and still active)
  function getPoolChores() {
    return chores.filter(c => {
      if (!c.pool || c.assigned_to) return false;
      // Don't show expired pool chores
