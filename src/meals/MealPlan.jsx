  ...
  async function loadData() {
    ...
    const [planRes, mealRes] = await Promise.all([
      supabase.from('meal_plan').select('*, meals(*)').eq('family_id', familyId)
        .gte('plan_date', startDate).lte('plan_date', endDate).order('plan_date'),
      supabase.from('meals').select('*').eq('family_id', familyId).order('title'),
    ]);
        ...
        <div style={styles.overlay}>
          <div style={styles.pickerModal}>
            ...
            <div style={styles.quickRow}>
              <input type="text" placeholder="Eller skriv fritt, t.ex. Restomat" value={quickText}
                onChange={e => setQuickText(e.target.value)} style={{ ...S.input, flex: 1 }} />
              <button onClick={() => assignFreeText(pickingDay)} disabled={!quickText.trim()}
                style={{ ...S.button, ...S.buttonPrimary, padding: '10px 14px', opacity: quickText.trim() ? 1 : 0.5 }}>
                OK
              </button>
            </div>
            <MealBank meals={meals} familyId={familyId} memberId={member.id}
...
const styles = {
  ...
  dayRow: { padding: '12px 14px', borderRadius: 12, border: `1px solid ${C.borderLight}`, marginBottom: 6 },
  dayHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  dayName: { fontFamily: F.heading, fontSize: F.sizes.md },
  dayDate: { fontSize: F.sizes.xs, color: C.textMuted },
  mealAssigned: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  mealName: { fontSize: F.sizes.md, fontWeight: F.weights.semi, color: C.text },
  mealActions: { display: 'flex', gap: 6 },
  mealActionBtn: { padding: '0 12px', minHeight: 44, borderRadius: 8, border: `1px solid ${C.border}`, background: C.bgCard, fontSize: F.sizes.xs, color: C.textMuted, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2, WebkitTapHighlightColor: 'transparent' },
  mealEmpty: { display: 'flex', gap: 6 },
  addMealBtn: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, padding: '0 14px', minHeight: 44, borderRadius: 10, border: `1px dashed ${C.primary}`, background: C.primaryLight, color: C.primary, fontSize: F.sizes.sm, fontWeight: F.weights.semi, fontFamily: F.heading, cursor: 'pointer', WebkitTapHighlightColor: 'transparent', flex: 1 },
  randomBtn: { padding: '0 12px', minHeight: 44, borderRadius: 10, border: `1px solid ${C.border}`, background: C.bgCard, color: C.textMuted, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', WebkitTapHighlightColor: 'transparent' },
  ...
  pickerModal: { width: '100%', maxWidth: 500, maxHeight: '85vh', overflowY: 'auto', background: C.bgCard, borderRadius: '20px 20px 0 0', padding: 20 },
  pickerHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  pickerTitle: { fontFamily: F.heading, fontSize: F.sizes.lg, fontWeight: F.weights.bold, color: C.text, margin: 0 },
